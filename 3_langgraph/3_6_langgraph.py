# Code does not use langgraph, but is written in ReAct style
...
"""
        ...
        '''
        action_match.group(1) → This is the first captured group, i.e., the action name (Search, SerpSearch, Finalize). .lower() converts it to lowercase for easier matching (search, serpsearch, finalize).
        action_match.group(2) → This is the second captured group, i.e., the content inside brackets. .strip() removes any extra spaces from the start or end.
        '''
        action, arg = action_match.group(1).lower(), action_match.group(2).strip()

        '''
        If the action is search, it calls tool_search with arg as input.
        If it's serpsearch, it calls tool_serp.
        If it's finalize, it ends the loop and produces the final answer.
        '''
        if action == "search":
            obs = tool_search(arg)
